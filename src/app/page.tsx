  ...
  loading: () => (
    <div className="space-y-4">
      ...
      {Array.from({ length: 5 }).map((_, index) => (
        <div
          key={index}
          className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50 dark:border-slate-700/30 shadow-sm"
        >
...
export default function Home() {
  ...
  const [lastGeo, setLastGeoState] = useState<{
    lat: number;
    lon: number;
    label: string;
    updatedAt: string;
  } | null>(null);
  const [originalCurrentWeather, setOriginalCurrentWeather] =
    useState<CurrentWeather | null>(null);
  const [originalForecast, setOriginalForecast] = useState<Forecast | null>(
    null
  );
  ...
  const {
    ...
    queryKey: selectedCity
      ? queryKeys.current(selectedCity.lat, selectedCity.lon, "metric")
      : ["no-query"],
    queryFn: async () => {
  ...
  const {
    ...
    queryKey: selectedCity
      ? queryKeys.forecast(selectedCity.lat, selectedCity.lon, "metric")
      : ["no-forecast-query"],
    queryFn: async () => {
      if (!selectedCity) return null;
      if (typeof window === "undefined") return null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-all duration-500">
      ...
      {/* Modern Header - Moved outside main for Safari compatibility */}
                        ...
                        <ol className="space-y-2 list-decimal list-inside opacity-90 ml-2">
                          <li>
                            <span className="font-medium">
                              Try clicking "Use my location" again
                            </span>
                            <br />
                            <span className="opacity-75">
                              Some browsers may show the permission dialog
                              again.
                            </span>
                          </li>
                          <li>
                            ...
                            <ul className="mt-1 space-y-1 list-disc list-inside opacity-75 ml-4">
                              <li>
                                Click the lock icon (🔒) or site info icon in
                                your browser's address bar
                              </li>
                              <li>Open "Site settings" or "Permissions"</li>
                              <li>
                                Find "Location" and change from "Block" to
                                "Allow"
                              </li>
              ...
              !isLoadingForecast && (
                <div className="space-y-8">
                      ...
                      <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl rounded-3xl p-6 border border-slate-200/50 dark:border-slate-700/50 shadow-lg h-full flex flex-col">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button
                            ...
                            aria-label={
                              showMap ? strings.hideMap : strings.showMap
                            }
                          >
                            <svg
                  ...
                  <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl rounded-3xl p-8 border border-slate-200/50 dark:border-slate-700/50 shadow-lg">
                    <ForecastList forecasts={forecasts} units={units} />
                  </div>

                  {/* Offline indicator for cached data */}
                  {forecastData &&
                    forecastData._cached &&
                    forecastData._cachedAt && (
                      <div className="mt-6">
                        <OfflineIndicator
